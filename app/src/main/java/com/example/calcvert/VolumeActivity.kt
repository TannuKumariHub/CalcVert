...
import com.example.calcvert.databinding.ActivityVolumeBinding

class VolumeActivity : AppCompatActivity() {
    val binding by lazy {
        ActivityVolumeBinding.inflate(layoutInflater)
    }
    private val units = arrayOf("Cubic meter","cubic centimeter","Liter","Centiliter","Cubic foot","Cubic inch","cubic millimeter","Milliliter","Centiliter")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.spinnerFromVolume.adapter = adapter
        binding.spinnerToVolume.adapter = adapter

        // Set up button click listener
        binding.btnconvertVolume.setOnClickListener {
            convertUnits()
        }
    }

    private fun convertUnits() {
        val fromUnitIndex = binding.spinnerFromVolume.selectedItemPosition
        val toUnitIndex = binding.spinnerToVolume.selectedItemPosition

        if (fromUnitIndex == toUnitIndex) {
            binding.textResultVolume.text = "Please select different units."
            return
        }
        ...
        try {
            ...
            binding.textResultVolume.text = "$inputValue ${units[fromUnitIndex]} is $result ${units[toUnitIndex]}"
        } catch (e: NumberFormatException) {
            binding.textResultVolume.text = "Invalid input. Please enter a valid number."
        }
    }
