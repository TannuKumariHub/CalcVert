...
import com.example.calcvert.databinding.ActivityUnitConverterBinding

class LengthActivity : AppCompatActivity() {
    val binding by lazy {
        ActivityUnitConverterBinding.inflate(layoutInflater)
    }


    private val units = arrayOf("Meter", "Mile", "Centimeter", "Foot", "Kilometer","Millimeter","Decimeter","Inch","Yard")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnconvertLength.setOnClickListener {
            convertUnits()
        }
    }

    private fun convertUnits() {
        val fromUnitIndex = binding.spinnerFrom.selectedItemPosition
        val toUnitIndex = binding.spinnerTo.selectedItemPosition

        if (fromUnitIndex == toUnitIndex) {
            binding.textViewResult.text = "Please select different units."
            return
        }
        ...
        try {
            ...
            binding.textViewResult.text = "$inputValue ${units[fromUnitIndex]} is $result ${units[toUnitIndex]}"
        } catch (e: NumberFormatException) {
            binding.textViewResult.text = "Invalid input. Please enter a valid number."
        }
    }
}